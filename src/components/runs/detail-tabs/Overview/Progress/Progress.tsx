...
export function ProgressCollapsible({ runId }: Props) {
	...
	const counts = useMemo(() => (dag ? computeStepCounts(dag.nodes) : undefined), [dag?.nodes]);
	if (isPending) return <Skeleton className="h-[120px] w-full" />;
	if (!dag || !counts) return null;
	...
	return (
		<div className="flex flex-col gap-1">
			...
			<div className="flex items-center gap-1">
				<ExecutionStatusIcon status={representativeStatus} className="animate-none" />
