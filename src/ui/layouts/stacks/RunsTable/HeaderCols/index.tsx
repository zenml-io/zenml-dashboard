...

    {
      ...
      renderRow: (run: TRun) => (
        <Paragraph style={{ justifyContent: 'center', backgroundColor: run.status === "Finished" ? "#431D93" : run.status === 'In Progress' ? '#ffff00' : "#FF5C93", borderRadius: '50%', height: '25px', width: '25px', paddingTop: '3px', textAlign: 'center' }}>
            {run.status === "Finished" ? <icons.check color={iconColors.white} size={iconSizes.sm} /> :  run.status === 'In Progress' ? <icons.alertTriangle color={iconColors.white} size={iconSizes.sm} /> : <icons.close color={iconColors.white} size={iconSizes.sm} />} 
        </Paragraph>
      ),
    },
