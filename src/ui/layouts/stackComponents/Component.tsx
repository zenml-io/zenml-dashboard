...
import { Box, FlexBox, icons, Paragraph, SearchInputField } from '../../components';
import { iconSizes, iconColors } from '../../../constants';
import { useSelector, useDispatch, useHistory } from '../../hooks';
import { projectSelectors, stackComponentSelectors } from '../../../redux/selectors';
...
const Component = () => {
    ...
    const sectionStyle = { width: '207px', padding: '15px 29px 11px 23px', alignItems: 'center', borderRadius: '4px', cursor: 'pointer' }
    ...
    const stacks = stackComponentsTypes?.filter((e) => {
        const searchFil = () => {
          if (search) { return e?.includes(search) }
          if (!search) { return e }
        }
        return searchFil()
      })
    ...
    <Box style={{ borderRight: '1px solid #A8A8A8', padding: '0 50px 0 33px', marginTop: '12rem' }} >
        ...
        <Box style={{ marginTop: '-22px' }} >
            <SearchInputField placeholder='Search' value={search ? search : ''} onChange={(e: string) => setSearch(e)} />
        </Box>
